...
function template($name) {
	...
	if($_SGLOBAL['mobile']) {
		...
		if($_SC['debug'] || !file_exists($objfile)) {
...
function avatar($uid, $size='small', $returnsrc = FALSE) {
	...
	$avatarfile = avatar_file($uid, $_size);

    return $returnsrc ? UC_API.'/data/avatar/'.$avatarfile : 
        $width && $height ? '<img src="'.UC_API.'/data/avatar/'.$avatarfile.'" onerror="this.onerror=null;this.src=\''.UC_API.'/images/noavatar_'.$_size.".gif'\" width=\"${width}\" height=\"${height}\" >" : 
        '<img src="'.UC_API.'/data/avatar/'.$avatarfile.'" onerror="this.onerror=null;this.src=\''.UC_API.'/images/noavatar_'.$size.'.gif\'">';
}
