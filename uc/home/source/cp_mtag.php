...
if($_GET['op'] == 'manage') {
    ...
    //群主
    if($mtag['allowview']){
        $mtag_creator = $_SGLOBAL['db']->fetch_array($_SGLOBAL['db']->query("SELECT uid, username FROM ".tname('tagspace')." main WHERE main.tagid='$tagid' AND grade='9'"));
        //var_dump($mtag_creator);exit;
        realname_set($mtag_creator['uid'], $mtag_creator['username']);
    }
    ...
    if($mtag['allowview']) {
        $count = $_SGLOBAL['db']->result($_SGLOBAL['db']->query("SELECT COUNT(*) FROM ".tname('tagspace')." main WHERE main.tagid='$tagid'"),0);
        if($count) {
            $query = $_SGLOBAL['db']->query("SELECT * FROM ".tname('tagspace')." WHERE tagid='$tagid' LIMIT 0,9");
            while ($value = $_SGLOBAL['db']->fetch_array($query)) {
                //实名
