...
if($_SGLOBAL['supe_uid']) {
	showmessage('do_success', usr_url('space'), 0);
}

$refer = empty($_GET['refer'])?rawurldecode($_SCOOKIE['_refer']):$_GET['refer'];
#preg_match("/(admincp|do|cp)\.php\?ac\=([a-z]+)/i", $refer, $ms);
$ms = usr_getParam($refer);
if($ms) {
	if($ms['name'] != 'cp' || $ms['query']['ac'] != 'sendmail') $refer = '';
}
if(empty($refer)) {
	$refer = usr_url('space', array('do'=>'home'));
}
...
if($uid && $code && !$reward['credit']) {
	...
	$url_plus = usr_param(array("uid"=>$uid, "app"=>$app, "code"=>$code));
} elseif($uid && $invite) {
	include_once(S_ROOT.'./source/function_cp.php');
	$invitearr = invite_get($uid, $invite);
	$url_plus = usr_param(array("uid"=>$uid, "invite"=>$invite));
}
...
if(submitcheck('loginsubmit')) {
	...
	if(empty($_POST['username'])) {
		showmessage('users_were_not_empty_please_re_login', usr_url('do', array('ac'=>$_SCONFIG['login_action'])));
	}
	...
	if(!$passport = getpassport($username, $password)) {
		showmessage('login_failure_please_re_login', usr_url('do', array('ac'=>$_SCONFIG['login_action'])));
	}
	...
	if(empty($_POST['refer'])) {
		$_POST['refer'] = usr_url('space', array('do'=>'home'));
	}
	
	realname_get();
	
	showmessage('login_success', $app?usr_url("userapp", array("id"=>$app)):$_POST['refer'], 1, array($ucsynlogin));
}
